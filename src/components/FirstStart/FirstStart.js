...
function FirstStart() {
  return (
    <div className="firststart">
      ...
      <p className="firststart__subtitle">
        Выполни регистрацию или вход прямо сейчас!
      </p>
      <div className="firststart__links">
        <Link className="firstart_link_reg" to="/reg">
          <button className="firststart__btnReg">
            Зарегестрироваться бесплатно
          </button>
        </Link>
        <Link to="/login" className="firststart__btnLog">
          Войти
        </Link>
      </div>
      <p className="firststart__posttitle">
        Используя сервис «О!идея» ВЫ СОГЛАШАЕТЕСЬ С ПРАВИЛАМИ ИСПОЛЬЗОВАНИЯ,
        КОТОРЫЕ ВКЛЮЧАЮТ В СЕБЯ НАСТОЯЩУЮ ПОЛИТИКУ КОНФИДЕНЦИАЛЬНОСТИ. 2024 ©
        Все права защищены.
      </p>
    </div>
  );
